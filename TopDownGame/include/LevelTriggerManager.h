...
{
public:
	Trigger(int px, int py, int sx, int sy, int neededLevel, std::function<void()> func, bool isDestroyable = false) : InteractableObject(nullptr, px, py, sx, sy),
		neededLevel(neededLevel), func(func), isDestroyed(false), isDestroyable(isDestroyable) {
	}
