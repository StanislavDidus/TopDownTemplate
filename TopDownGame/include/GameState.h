...
struct ExploringState : public GameState
{
	...
	void onUpdate(Tmpl8::Game& game, float deltaTime)  override
	{
		...
		game.questSystem->update(deltaTime);
		
		game.triggers.erase(std::remove_if(game.triggers.begin(), game.triggers.end(), [this](const std::shared_ptr<Trigger>& t) {return t->isDestroyed; }), game.triggers.end());
	}
...
struct BattleState : public GameState
{
		...
		advantage(advantage)
	{
		...
		EventBus::Get().AddListener(EventType::ATTACK, std::function<void(int)>([this](int value) { ShowMessage("Dealt damage: ", value); }));
		EventBus::Get().AddListener(EventType::HIT, std::function<void(int)>([this](int value) { ShowMessage("Taken damage: ", value); }));
		EventBus::Get().AddListener(EventType::HEALED, std::function<void(int)>([this](int value) { ShowMessage("Hp healed: ", value); }));
		EventBus::Get().AddListener(EventType::STUNNED,std::function<void(int)>([this](int value) { ShowMessage("Stunned for: ", value); }));
	}
		...
		for (const auto& w : weapons)
		{
			if (enemiesInBattle.size() > 0)
			{
				if (auto& e = enemiesInBattle[0].lock())
				{
					...
					if (e->getHp() <= 0)
					{
						EventBus::Get().Push(EventType::KILLED, e->getTag());
