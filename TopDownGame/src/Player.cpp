...
Player::Player(Tmpl8::Sprite* sprite, int px, int py, int sx, int sy, Map* map) : Entity(sprite, px, py, sx, sy, 100, 0, 0, 1.f), map(map), fx(0), fy(0)
{
...
void Player::update(float deltaTime)
{
	...
	if (!map->intersects(nx, py + 36, 48, 36))
		px = nx;
	if (!map->intersects(px, ny + 36, 48, 36))
		py = ny;
