...
void Surface::PrintScaled(char* a_String, int x1, int y1, float scaleX, float scaleY, int width, Pixel color)
{
	if (!fontInitialized)
	{
		InitCharset();
	...
	for (int i = 0; i < str_list.size(); i++)
	{
		...
		for (int j = 0; j < s.size(); j++, t += letterSizeX)
		{
			...
			if (j == 0 && (t - m_Buffer) - (y1 + height) * m_Pitch + s.size() * letterSizeX > x1 + width)
			{
