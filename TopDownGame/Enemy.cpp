...
Enemy::Enemy(Tmpl8::Sprite* sprite, int px, int py, int sx, int sy, Map* map, int neededLevel, int health, int damage, float attackSpeed, int money, int exp) : Entity(sprite, px, py, sx, sy, health, money, exp), map(map),
damage(damage), attackSpeed(attackSpeed), lastAttackTime(0.f), neededLevel(neededLevel), timer(0.f), x(0.f), y(0.f)
{
	srand(time(NULL));
}
